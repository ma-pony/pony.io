...
今天重点扫描的是 **AI Coding / Agent / MCP / AI Infra** 这条线，因为它正在从“模型能力展示”进入“可购买、可部署、可治理”的工程化阶段。

实际检查过的来源：

- **开源 / GitHub**：GitHub Trending daily、Python / TypeScript Trending、GitHub Topics：`ai-agent`、`mcp`、`ai-coding`、`coding-agent`、`llm-agents`。
- **官方发布 / 一手信息**：OpenAI RSS、Anthropic News、Google AI Blog、NVIDIA Blog、Mistral News、Hugging Face Blog、LangChain Blog、LlamaIndex Blog、Vercel Blog、Cursor Blog。
- **论文 / 研究**：arXiv API，重点查 `cs.AI`、`cs.CL`、`cs.SE` 里 Agent、Coding Agent、MCP、Sandbox、PR Review 相关论文。
- **社区 / 产品信号**：Hacker News Algolia、Product Hunt RSS。
- **受限来源**：OpenAI 官网 HTML 访问返回 403，但 RSS 可用；Product Hunt AI 分类页 Cloudflare 403，但 RSS 可用；Reddit JSON 多个子版块解析失败，未采用；X/Twitter 本次未直接抓取，避免把不可验证传播信号当事实。

判断标准：不追热点名词，优先看 **能不能变成产品、能不能提升研发效率、能不能降低 Agent 落地风险、能不能给社媒带来高传播选题**。
